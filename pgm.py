...
#===BG Image===================
	login_menu.bg= PhotoImage(file="pic1.png")
	...
	#title_label=Label(login_menu,relief=GROOVE,text="book RENTAL SYSTEM LOGIN PORTAL",font=('Times new roman',20,'bold'))
	#title_label.grid(row=0,column=0,columnspan=2,pady=20)

	#id
	id_label=Label(login_menu,relief=GROOVE,text="Your ID:",font=('arial',20,'bold'))
	id_label.grid(row=9,column= 30)
	id_input_login=Entry(login_menu)
	id_input_login.grid(row=9,column=43,columnspan=1,pady=20)
	
	#password
	password_label=Label(login_menu,relief=GROOVE,text="Password:",font=('arial',20,'bold'))
	password_label.grid(row=10,column= 30)
	password_input_login=Entry(login_menu, show="*")
	password_input_login.grid(row=10,column=43,columnspan=1,pady=20)
	
	#login button
	loginbutton1=Button(login_menu,command=login_check,text=" Login ",bg='white',fg='black',height=2,width=14,font=k_font)
	loginbutton1.grid(row=12,column= 32,columnspan=2,pady= 2)
	
	#register button
	registerbutton=Button(login_menu,command=register_in,text=" Register ",bg='white',fg='black',height=2,width=14,font=k_font)
	registerbutton.grid(row=12,column= 36,columnspan=2,pady=2)
	#feedback button
	feedbackbutton=Button(login_menu,command=feedback_read,text=" Feedback ",bg='white',fg='black',height=2,width=14,font=k_font)
	feedbackbutton.grid(row=14,column =32,columnspan=2,pady= 2)
	#admin button
	adminbutton=Button(login_menu,command=admin_in,text=" Admin Login ",bg='white',fg='black',height=2,width=14,font=k_font)
	adminbutton.grid(row=14,column = 36,columnspan=2,pady= 2)
...
def Admin_Opt():
		...
		addbutton=Button(opt_menu,command=add_book,text=" Add Camera ",bg='grey',fg='white',height=1,width=12,font=k_font)
		delbutton=Button(opt_menu,command=del_book,text=" Remove Camera ",bg='grey',fg='white',height=1,width=12,font=k_font)
		backbutton=Button(opt_menu,command=reopen_login,text=" Log out ",bg='grey',fg='white',height=1,width=12,font=k_font)

		addbutton.grid(row=4,column=5)
...
def add_book():
	...
	add_menu.minsize(450,350)
	add_menu.maxsize(450,350)
	add_menu.resizable(0,0)
	k_font = tkinter.font.Font(family='Times new roman', size=10, weight=tkinter.font.BOLD)

	book_id_label=Label(add_menu,text="Camera ID (Should be of atleast length 5)")
	book_label=Label(add_menu,text="Camera Name")
	author_label=Label(add_menu,text="Company Name")
	book_id=Entry(add_menu)
	book_name=Entry(add_menu)
	author_name=Entry(add_menu)
	addbutton1=Button(add_menu,command=add_check,text=" Add Camera ",bg='black',fg='white',height=1,width=10,font=k_font)
...
def add_check():
	...
	if len(b_name)==0:
		tkinter.messagebox.showinfo("Add Camera","You did not type a Camera name *_*😁")
		add_menu.lift()
		return(add_book)
...
def del_book():
	...
	delbutton1=Button(del_menu,command=del_check,text="Remove Camera",bg='black',fg='white',height=1,width=15,font=k_font)
...
def del_check():
	...
	if(pos == -1):
		...
		w1 = l1.split('|')
		if(w1[3] == 'N'):
			tkinter.messagebox.showinfo("Delete","Camera currently borrowed. Please try another book")
			del_menu.destroy()
			return(del_book)
...
def Main_Menu():
	...
	#Bunch of labels
	status = Label(base,text=("Date and time logged in: " + current_time),bd=1,relief=SUNKEN,anchor=W,bg='white')
	orionLabel=Label(base, text="CAMERA RENTAL STORE",bg='black',fg='white',font=("Gabriola", "50","bold","italic"))
	welcomeLabel=Label(base,text=("Welcome! "+id),font=("Freestyle Script","50","bold"))
	imageLibrary = PhotoImage(file="pic.gif")
	topFrame=Frame(base)
	...
	#Buttons
	borrow_but=Button(bottomFrame,bg="grey",fg="white",text="Borrow Camera",font=in_font,height=5,width=15,command=borrow_in)
	return_but=Button(bottomFrame,bg="grey",text="Return Camera",font=in_font,height=5,width=15,command=return_in)
	search_but=Button(bottomFrame,bg="grey",fg="white",text="Search for Camera",font=in_font,height=5,width=15,command=search_in)
	feedback_but=Button(bottomFrame,bg="grey",text="Feedback",font=in_font,height=5,width=15,command=feedback_in)
...
def borrow_in():
	...
	borrow_list1.configure(background="grey")
	borrow_list2.configure(background="grey")
	borrow_list3.configure(background="grey")
	borrow_label1=Label(borrow_menu,text="<<< Please enter the Camera ID that you wish to borrow >>>",font=("Times", "20","bold","italic"),bg='grey',fg='white')
	borrow_label=Label(borrow_menu,text="Id")
	borrow_label2=Label(borrow_menu,text="Camera")
	borrow_label3=Label(borrow_menu,text="Company")
	borrow_label4=Label(borrow_menu,text="Stocks")

	borrow_entry1=Entry(borrow_menu,width=50)
	borrow_button1=Button(borrow_menu,text="Borrow",command=borrow_check,font=("Times new roman","10","bold"),bg='grey',fg='white')
...
def borrow_check():
	...
	for l in f:
		...
		if l[0] ==  id:
			count += 1
	if count >= 2:
		tkinter.messagebox.showinfo("Borrow", "Cannot borrow more than 2 Camera at a time")
		borrow_menu.destroy()
		...
		if len(bbook) == 0:
			tkinter.messagebox.showinfo("Borrow","You did not type anything *_*")
			borrow_menu.lift()
			return(borrow_in)

		pos = binary_search('Bindex.txt', bbook)
		if pos == -1:
			tkinter.messagebox.showinfo("Borrow","The Camera that you had entered is not in our database,sorry,please enter a different outfit")
			borrow_menu.lift()
		else:
			f2 = open('BData.txt', 'r+')
			f2.seek(pos)
			...
			if(w2[3] == 'Y'):
				...
				f3.close()
				key_sort('Record.txt')
				Done2=tkinter.messagebox.askyesno("Borrow","Do you want to borrow another Camera?")
				if Done2==True:
					borrow_menu.destroy()
					borrow_in()
				else:
				...
				tkinter.messagebox.showinfo("Borrow","This Camera is currently unavailable, please select another outfit")
				borrow_menu.lift()
...
def return_in():
	...
	return_list.configure(background="grey")
	return_list1.configure(background="grey")
	return_list2.configure(background="grey")
	return_list3.configure(background="grey")
	return_label=Label(return_menu,text="Id")
	return_label2=Label(return_menu,text="Camera Name")
	return_label3=Label(return_menu,text="Company")
	return_label4=Label(return_menu,text="	Stocks")

	return_button1=Button(return_menu,text="Return",command=return_check,font=("Times new roman","10","bold"),bg="grey",fg="white")
	return_entry1=Entry(return_menu,width=50)
	return_label1=Label(return_menu,text=" Please enter the Camera ID that you wish to return ",font=("Times", "12","bold","italic"),bg="grey",fg="white")
			...
			if(w1[3] == 'N'):
				...
				for l2 in lines:
					...
					if l3[1] == bbook and l3[0] == id:
						...
						f3.write(l2)
				f3.close()
				Done3=tkinter.messagebox.askyesno("Return","Do you want to return another Camera?")
				if Done3==True:
					f1.close()
					return_menu.destroy()
					return_in()
				...
				tkinter.messagebox.showinfo("This Camera has been returned, please select another Camera")
			f1.close()
	else:
...
def search_in():
	...
	search_label1=Label(search_menu,text="Search through our database to check if your desired Camera is available",font=("Times", "12","bold","italic"),bg='grey',fg='white')
	search_label1.pack(side=TOP)

	search_entry = Entry(search_menu,width=50)
	search_entry.pack(side=TOP)

	search_button=Button(search_menu,text="Search",command=search_check,font=("Times new roman","10","bold"),bg="grey",fg="white")
...
def search_check():
	...
	if (pos == -1):
		tkinter.messagebox.showinfo("Search","Sorry,this Camera does not exist in our database")
	else:
		search_menu2=Tk()
...
def feedback_in():
	...
	feedback_label=Label(feedback_menu,text= "We improve from your valuable feedback.Thank you!",font=("Monotype corsiva","15","italic"),bg='grey',fg='white')
	button1=Button(feedback_menu, text="Submit feedback",command=feedback_check,font=("Times new roman","10","bold"),bg="grey")
...
def feedback_read():
	...
	read_feedback_menu.geometry("1280x780")
